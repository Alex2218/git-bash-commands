...
git commit --amend --no-edit # для того, чтобы не открывался редактор для ввода описания коммита
```

### Отмена коммитов

``` bash
git reset --hard # полное удаление коммита
git revert HEAD --no-edit # создать новый коммит, отменяющий изменения последнего коммита (без запуска редактора сообщения)
git reset --hard 75e2d51 # вернуть репозиторий в состояние коммита с указанным хешем
git reset --hard HEAD~ # означает "один коммит от последнего коммита". Если бы мы хотели удалить два последних коммита, то могли бы написать HEAD~2.
```

### Удаление файла и перейминования (просто удалить отслеживаемый файл из папки недостаточно, нужно сделать его неотслеживаемым и отправить коммит)
...
git mv # после чего пишем текущее название файла, а затем новое (original_file_name  new_file_name)
```
...
```
git bisect start # для запуска поиска ошибок
git bisect bad # сообщить Git, что текущий коммит сломан
git bisect good "v1.0" #вы должны указать, когда было последнее известное рабочее состояние
git bisect reset #для того, чтобы вернуть HEAD туда, где он был до начала поиска
```
...
``` bash
git log # показать список всех виполнених коммитов
git log -2 # показать последние 2 коммита
git log -2 --stat # показать последние 2 коммита и статистику внесенных ими изменений
git log -p # флаг -p выводит диф для каждого коммита
...
git blame #путь до файла (пример: "git blame INFO.md"   "5120bea3 (tirion 2020-09-17 18:04:19 -0400 2) new line")
```
...
``` bash
git grep line # ответ: "NFO.md:new line"

# Флаг i позволяет искать без учета регистра
git grep -i hexlet # ответ : "README.md:Hello, Hexlet! How are you?'

# Поиск в конкретном коммте
...
``` bash
git remote add <название ветки репозитория и ссылка с Git на репозиторияй>  #добавить выбраный репозиторий
git remote -v # показать список удалённых репозиториев, связанных с этим
git remote remove <репозиторий> #удалить выбраный репозиторий
...
``` bash
git clean -f  или git clean -fd # удалить из репозитория все неотслеживаемые файлы
logout # закрить Git Bash
git restore #окончательно отменяет изменения в выбранных файлах
git restore --staged <file>перемещает изменения, сделанные в файле, из индекса в рабочую директорию
git branch # узнать на каком коммите мы находимся
vim <название файла> # Редактируем и сохраняем файл
...
git stash pop # везращает спрятанные файли (изменения)
git stash list #показивает сколько файлов мы спрятали
```
...
git merge upstream/master # вливаем ветку master удалённого репозитория upstream в свою ветку master[main c30febe] добавил 'перуейминование файла'
